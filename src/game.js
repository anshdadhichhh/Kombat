...
import { AssetLoader, normalizeObject } from './assetLoader.js';
...
export class FightingGame {
  constructor(container = document.body) {
    ...
    // Keep camera fixed so it does not look like the camera is moving instead of the character.
  ...
  async init() {
    ...
    try {
      ...
      this.hideLoading();
      console.log('Game ready. Controls: P1 A/D/W/S/J/K/L, P2 arrows/Numpad1/2/3');
    } catch (err) {
  ...
  addLights() {
    const hemi = new THREE.HemisphereLight(0xaec8ff, 0x302010, 1.6);
    this.scene.add(hemi);
    ...
    const rim = new THREE.PointLight(0x3366ff, 2.2, 20);
    rim.position.set(4, 3, -4);
    this.scene.add(rim);
  }

  async loadArena() {
    // Supports these exact filenames. Put ONE of them in public/assets/arena/.
    const arenaUrls = [
    ...
    for (const url of arenaUrls) {
      try {
        ...
        normalizeObject(arenaObject, 3.0);
        arenaObject.scale.multiplyScalar(3.0);
        arenaObject.position.set(0, 0, 0);
  ...
  async loadFighters() {
    ...
    this.p2 = new Fighter({
      id: 'P2', color: 0xff374f, startX: 2.4,
